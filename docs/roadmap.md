...
- multi-tenant Phase 4 messaging-first interface: agent records carry `is_main` and `messaging` fields validated against `policy.allowed_messaging`; per-pod `messaging-bridge-{email,signal}` sidecar images and Quadlet templates rendered on demand; `messaging-bridge-whatsapp` ships as a stub (Meta Business webhook ingress is Phase 5); `openclaw-runtime-router` replaces the Phase-0 idle stub with a strict verb-table dispatcher (`create-agent`, `list-agents`, `stop-agent`, `status`, `help`) over a pod-local two-socket message bus; new admin verbs `platformctl agent set-main` and `platformctl signal-link`; new agent-side flag `agentctl create-agent --messaging`; `concepts/messaging_interface.md` design doc and `how-to/enroll_messaging.md` walkthrough
...
29. Phase 3 — agent provisioning (done): `openclaw-provisioner` daemon at `/usr/local/bin/openclaw-provisioner` (Python single-file, `Type=simple`, restart-on-failure). Reads per-tenant `policy.yaml`, validates `agent_create` against allowed images / credentials / networks / volumes / quotas / forbidden flags, cross-checks credentials with the broker, renders agent Quadlets from `/var/lib/openclaw-platform/templates/agent_quadlet/`, then `daemon-reload` + `systemctl --user --machine=tenant_<tenant>@ start <tenant>-<agent>-pod.service`. Records each agent at `tenants/<tenant>/agents/<agent>.json` per the §15.4 object model in `design/multi_tenant_architecture.md`. Append-only audit log at `provisioner/audit.log`. Admin socket at `/run/openclaw-provisioner/admin.sock` (UID 0); per-tenant socket at `/run/openclaw-provisioner/tenants/<tenant>.sock` (chowned to `tenant_<tenant>`) — bind-mounted into the openclaw-runtime container at `/run/agentctl/agentctl.sock` so the in-container `agentctl` CLI can self-provision. `platformctl agent create | list | inspect | start | stop | delete` and `platformctl policy show` admin subcommands. `platformctl tenant create` now writes a real default `policy.yaml`. `how-to/create_an_agent.md` walkthrough. Still planned: `agentctl create-env`, `attach-storage`/`detach-storage`, per-agent cgroup enforcement, messaging-driven creation.
30. Phase 4 — messaging-first interface (done): per-pod messaging-bridge sidecars (real `email` + `signal`, stub `whatsapp`); agent-record `is_main` and `messaging` fields enforced against `policy.allowed_messaging`; provisioner renders `agent-messaging-bridge-<transport>.container.tmpl` only for the requested transports; `openclaw-runtime-router` (verb table: `create-agent`, `list-agents`, `stop-agent`, `status`, `help`) replaces the Phase-0 idle stub; per-agent `${PLATFORM_ROOT}/tenants/<tenant>/runtime/agents/<agent>/msgbus/` host directory shared between the runtime (`inbound.sock`) and each bridge (`outbound-<transport>.sock`); `platformctl agent set-main` (admin-only; per-tenant socket refuses) and `platformctl signal-link` helpers; `agentctl create-agent --messaging`; `concepts/messaging_interface.md` and `how-to/enroll_messaging.md`. WhatsApp ingress (Meta Business webhook + cloudflared `messaging-webhook` route) remains Phase 5; LLM-backed free-text dispatch is also Phase 5+.
31. Phase 5 — production hardening (planned): backups, restore tests, host rollback tests, tenant deletion tests, credential revocation tests, SELinux review, network egress policy, resource quotas, logging/audit, WhatsApp Business webhook ingress (cloudflared `messaging-webhook` ingress class generation, per-tenant public hostname allocation).
