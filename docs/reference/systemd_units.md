...
### backup.timer
- **Path**: `/usr/lib/systemd/system/backup.timer`
- **Type**: timer
- **Purpose**: Triggers `backup.service` daily to run the host backup container.
- **Triggers**: Fires once per day (`OnCalendar=daily`); `Persistent=true` catches up a missed run after the host was powered off.
- **Implements**: `backup.service` (generated by Quadlet from `backup.container`)
- **Enabled at build time?**: Yes

### backup.service
- **Path**: Generated by the Quadlet generator from `/usr/share/containers/systemd/backup.container`
- **Type**: oneshot container service
- **Purpose**: Pulls and runs the `backup-container` image to perform the backup task. The container exits cleanly when the task completes.
- **Triggers**: Activated by `backup.timer`; can also be started manually with `systemctl start backup.service`.
- **Enabled at build time?**: No (triggered by timer)
- **Notes**: The container receives `/var/lib/openclaw-platform` as a read-only volume. Real backup logic replaces `backup_stub.sh`.
